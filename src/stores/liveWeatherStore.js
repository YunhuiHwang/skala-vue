...
// 작성목적 : 대표 도시 8곳의 실시간 날씨를 OpenWeatherMap 에서 조회해 보관한다.
// 변경사항 :
//   - 하루 한 번만 실제로 API를 호출하도록 오늘 날짜를 키에 포함한 localStorage 캐싱 적용
//     (자정이 지나면 캐시 키가 자동으로 바뀌므로 별도 만료 로직 없이 새로 조회된다)
//   - loading / error 상태를 노출해 지도/카드 쪽에서 로딩 UI를 표시할 수 있게 함
